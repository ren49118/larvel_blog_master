              ...
              <table class="table table-hover table-bordered mt-3" id="sampleTable">
                ...
                <tbody>
                  ...
                  <tr>
                    <th scope="row">{{$i++}}</th>
                    <td>{{$row->name}}</td>
                    <td><img src="{{asset($row->photo)}}" width="70px" height="70px"></td>
                    <td>
                      <a href="{{route('brands.edit',$row->id)}}" class="btn btn-warning text-light">Edit</a>
                      <div class="d-inline-block">
                          <form method="POST" action="{{route('brands.destroy',$row->id)}}" onsubmit="return confirm('r u sure')">
                            @csrf
                            @method('DELETE')
